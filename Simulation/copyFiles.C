...
void copyFiles(TString targetDir)
{
  ...
  gSystem->Exec(Form("%s%s/AODtrainCustom.C %s/AODtrainCustom.C",
                     cp.Data(), localDir.Data(), targetDir.Data()));
/*
  gSystem->Exec(Form("%s$DEVPHYS/PWG/muondep/AddTaskMuonCDBConnect.C %s/AddTaskMuonCDBConnect.C",
  ...
  gSystem->Exec(Form("%s$ALICE_ROOT/ANALYSIS/ESDfilter/macros/AddTaskESDFilter.C %s/AddTaskESDFilter.C",
                     cp.Data(), targetDir.Data()));
*/  
}
