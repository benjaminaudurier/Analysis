...
void AliAnalysisTaskGenTuner::UserExec(Option_t *)
{
  ...
  //__________fill the MC part if running on MC
  TArrayD weight;
  if (MCEvent()) 
  {
...
//      if (mctrack->Y() < -4. || mctrack->Y() > -2.5) continue;
      ...
      //__________compute the weights for all primary particles (other weights are 0)
  ...
  for (Int_t i = 0; i < aod->GetNumberOfTracks(); i++)
  {
    ...
    if (!fMuonTrackCuts->IsSelected(track) || (fPtCut > 0. && pT < fPtCut) || (MCEvent() && mcLabel < 0)) continue;
