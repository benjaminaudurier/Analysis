...
//    Double_t mean = GetValue(name);
    ...
    Int_t nofvalidResults(0);
    Int_t nofTests(0);
    if ( fSubResults ) nofTests = fSubResults->GetEntries();
    ...
    while ( ( r = static_cast<AliAnalysisMuMuResult*>(next()) ) )
    {
      if ( IsIncluded(r->Alias()) && r->HasValue(name) )
      {
        ...
        Double_t test = r->GetErrorStat("NofJPsi")*100./r->GetValue("NofJPsi");
        Double_t testChi = r->GetValue("FitChi2PerNDF");
        if ( !(err>0.0 ) || (test < 0.01) || (testChi > 2.5) ) continue; // If the error is not correct (fit not good) of the error on the NofJpsi is very small(fit problems) we skip the subresult. This works both for <pT> and Minv results.
...
//        d += ( (r->GetValue(name) - mean)*(r->GetValue(name)-mean) / e2);
    ...
    if ( nofvalidResults != nofTests ) AliInfo(Form("Nof used subresults = %d (%f2.2 %%)",nofvalidResults,nofvalidResults*100./nofTests));
  ...
  if ( fResultMergingMethod == kMean )
  {
    ...
    while ( ( r = static_cast<AliAnalysisMuMuResult*>(next()) ) )
    {
      if ( IsIncluded(r->Alias()) && r->HasValue(name) )
      {
        ...
        if ( !(r->GetErrorStat(name)>0.0 ) || (test < 0.01) || (testChi > 2.5) ) continue; /*e2 = TMath::Sqrt(r->GetValue(name));*/ // If the error is not correct (fit not good) of the error on the NofJpsi is very small(fit problems) we skip the subresult. This works both for <pT> and Minv results.
    ...
    if ( errorSum != 0.0 )
    {
      if ( (nofvalidResults != nofTests) && (nofTests > 1 )) AliInfo(Form("Nof used subresults = %d (%f2.2 %%)",nofvalidResults,nofvalidResults*100./nofTests));
                                       ...
                                       Double_t value, Double_t errorStat, Double_t rms) const
{
  ...
  else if ( TString(key).BeginsWith("MeanPt") )
  {
    std::cout << opt << Form("\t\t%20s %9.2f +- %5.2f (%5.2f %%) MeV/c",key,value*1E3,1E3*errorStat,
                             value != 0.0 ? errorStat*100.0/value : 0.0);
  ...
  else
  {
    if ( errorStat > 0.0 )
    {
      ...
      std::cout << opt << Form("\t\t%20s %9.3e +- %9.3e (%5.2f %%)",key,value,errorStat,
                               value != 0.0 ? errorStat*100.0/value : 0.0);
      if ( rms )
      {
        std::cout << Form(" RMS %9.3e (%5.2f %%)",rms,100.0*rms/value);
      }
    }
    else
    {
