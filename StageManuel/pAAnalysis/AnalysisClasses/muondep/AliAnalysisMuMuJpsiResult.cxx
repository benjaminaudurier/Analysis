...
fMinvRS("")
{
  ...
  if ( !name.Contains("PSICB2") && !name.Contains("PSINA60NEW") && !name.Contains("PSICOUNT") ) //To avoid adding things to the name of simu results
  {
    ...
    {
      if ( (fHisto->GetEntries()<30000) ) //Automatically rebin mpt histos when they have less than 30000 entries
      {
        fHisto->Rebin(2);
      }
    ...
    else if ( name.BeginsWith("MP2T") )
    {
//      fHisto->Rebin(2);

      name.ReplaceAll("MP2TPSIPSIPRIME","");
      isMPt = kTRUE;
    }
    name += "_";
...
//                            11
  ...
  bck->SetParLimits(1, -1., 3.);
  bck->SetParLimits(2, -1., 1.);
  bck->SetParLimits(3, -1., 1.);
...
//    }
  ...
  bckInit->SetParameters(fHisto->GetBinContent(bin)*2.,0.1,0.01,0.03);
  
  bckInit->SetParLimits(0,fHisto->GetBinContent(bin),fHisto->GetBinContent(bin)*5.);
  bckInit->SetParLimits(1,-5.,5.);

  SetFitRejectRange(2.7,3.4);

  TFitResultPtr fitResultInit = fHisto->Fit(bckInit,"SRN");
  ...
  if ( static_cast<int>(fitResultInit) ) ProcessBkgFit(fitResultInit,bckInit,"FitFunctionBackgroundVWG","SRN");
  //___________
  ...
  for ( Int_t i = 0; i < 4; ++i )
  {
    fitTotal->SetParameter(i, bckInit->GetParameter(i)); // *0.95 in Pb-p
  }
  
  fitTotal->SetParLimits(0,fHisto->GetBinContent(bin)*0.8,fHisto->GetBinContent(bin)*4.);
  
  fitTotal->SetParLimits(1, -1., 3.); // on in Pb-p
  fitTotal->SetParLimits(2, -1., 1.);
  fitTotal->SetParLimits(3, -1., 1.);
  
  
  bin = fHisto->FindBin(3.09);
  fitTotal->SetParameter(4, fHisto->GetBinContent(bin)); // norm
  fitTotal->SetParLimits(4, 0.5*fHisto->GetBinContent(bin),fHisto->GetBinContent(bin)); // norm
  
  fitTotal->SetParameter(5, 3.1); // mean
  fitTotal->SetParLimits(5, 2.95, 3.25);
  
  Double_t sigmaMin(0.06);
  fitTotal->SetParameter(6, 0.064); // sigma
  fitTotal->SetParLimits(6, sigmaMin, 0.075);
  ...
  fitTotal->SetParLimits(11, fHisto->GetBinContent(bin)*0.03,fHisto->GetBinContent(bin));
  //______________
  ...
  if ( (fitTotal->GetParameter(6) <= (sigmaMin+1.e-4)) && (fHisto->GetEntries() <= 30000.) ) // A fix for low stat bins where sigma reaches limit in some cases
  {
    std::cout << "Sigma Jpsi at limit. Refitting again changing sigmaPsiP range" << std::endl;
    
    sigmaMin = 0.040;
    fitTotal->SetParameter(6, 0.067);
    fitTotal->SetParLimits(6, sigmaMin, 0.07); //0.06- 0.08
...
void AliAnalysisMuMuJpsiResult::FitPSIPSIPRIMECB2POL2EXP()
{
  ...
  //___________ Further attempts to fit bkg if the first one fails
  if ( static_cast<int>(fitResultInit) ) ProcessBkgFit(fitResultInit,bckInit,"FitFunctionBackgroundPol2Exp","SRN");
  //___________
...
//  fitTotal->SetParLimits(6, 0.05, 0.15);
  ...
  Double_t sigmaMin(0.06);
  fitTotal->SetParameter(6, 0.067); // sigma
  fitTotal->SetParLimits(6, sigmaMin, 0.08);
...
void AliAnalysisMuMuJpsiResult::FitPSIPSIPRIMECB2POL4EXP()
{
  ...
  //___________ Further attempts to fit bkg if the first one fails
  if ( static_cast<int>(fitResultInit) ) ProcessBkgFit(fitResultInit,bckInit,"FitFunctionBackgroundPol4Exp","SRN");
  //___________
  ...
  Double_t sigmaMin(0.06);
  fitTotal->SetParameter(8, 0.07); // sigma
  fitTotal->SetParLimits(8, sigmaMin, 0.08);
  ...
  if ( (fitTotal->GetParameter(8) <= (sigmaMin+1.e-4)) && (fHisto->GetEntries() <= 30000.) ) // A fix for low stat bins where sigma reaches limit in some cases
  {
    std::cout << "Sigma Jpsi at limit. Refitting again changing sigmaPsiP range" << std::endl;
    
    sigmaMin = 0.040;
    fitTotal->SetParameter(8, 0.067);
    fitTotal->SetParLimits(8, sigmaMin, 0.07); //0.06- 0.08tal->SetParLimits(8, sigmaMin, 0.08); //0.06- 0.08
...
void AliAnalysisMuMuJpsiResult::FitPSIPSIPRIMENA60NEWVWG()
{
  ...
  fitTotal->SetParLimits(0,fHisto->GetBinContent(bin)*0.8,fHisto->GetBinContent(bin)*3.);

  fitTotal->SetParLimits(1, -1., 3.);
  fitTotal->SetParLimits(2, -1., 1.);
  fitTotal->SetParLimits(3, -1., 1.);
  
  bin = fHisto->FindBin(3.09);
  fitTotal->SetParameter(4, fHisto->GetBinContent(bin)); // norm
  fitTotal->SetParLimits(4, fHisto->GetBinContent(bin)*0.7,fHisto->GetBinContent(bin)); // norm
  
  fitTotal->SetParameter(5, 3.15); // mean
  fitTotal->SetParLimits(5, 2.98, 3.2);
  
  Double_t sigmaMin = 0.060;
  fitTotal->SetParameter(6, 0.065); // sigma 0.067
  fitTotal->SetParLimits(6, sigmaMin, 0.074); //0.06- 0.08
  ...
  bin = fHisto->FindBin(3.68);
  fitTotal->SetParameter(15, fHisto->GetBinContent(bin)*0.2); //kPsi'
  fitTotal->SetParLimits(15, fHisto->GetBinContent(bin)*0.03,fHisto->GetBinContent(bin)*0.5);
...
void AliAnalysisMuMuJpsiResult::FitPSIPSIPRIMENA60NEWPOL2EXP()
{
  ...
  bin = fHisto->FindBin(3.68);
  fitTotal->SetParameter(15, fHisto->GetBinContent(bin)*0.2); //kPsi' //0.5
  fitTotal->SetParLimits(15, fHisto->GetBinContent(bin)*0.03,fHisto->GetBinContent(bin)*0.5); //0.01 - 1
...
void AliAnalysisMuMuJpsiResult::FitPSIPSIPRIMENA60NEWPOL4EXP()
{
  ...
  if ( (fitTotal->GetParameter(8) <= (sigmaMin+1.e-4)) && (fHisto->GetEntries() <= 30000.) ) // A fix for low stat bins where sigma reaches limit in some cases
  {
    ...
    sigmaMin = 0.040;
    fitTotal->SetParameter(8, 0.067);
    fitTotal->SetParLimits(8, sigmaMin, 0.07); //0.06- 0.08
...
//  TString resultName(Form("MEANPTFIT_%salphalow=%5.2fnlow=%5.2falphaup=%5.2fnup=%5.2f",fitName.Data(),par[7],par[8],par[9],par[10]));
  ...
  fitMeanpt->SetParameter(12, 3.1);
  fitMeanpt->SetParLimits(12, 2.0,4.);
  
  for ( Int_t i = 0; i < 3; ++i )
  {
    fitMeanpt->SetParameter(i + 13, bck->GetParameter(i)*0.95);
  }
  ...
  Set("FitChi2PerNDF",fitMeanpt->GetChisquare()/fitMeanpt->GetNDF(),0.0);
  Set("FitNDF",fitMeanpt->GetNDF(),0.0);
  
  Set("MPtBkG0",fitMeanpt->GetParameter(13),fitMeanpt->GetParError(13));
  Set("MPtBkG1",fitMeanpt->GetParameter(14),fitMeanpt->GetParError(14));
  Set("MPtBkG2",fitMeanpt->GetParameter(15),fitMeanpt->GetParError(15));
...
//  fitMeanpt->SetParLimits(12, 2.0,4.);
  ...
  Double_t psipPtLim = 10.;
  fitMeanpt->SetParLimits(17, 2.,psipPtLim);
  ...
  Set("MPtBkG1",fitMeanpt->GetParameter(14),fitMeanpt->GetParError(14));
  Set("MPtBkG2",fitMeanpt->GetParameter(15),fitMeanpt->GetParError(15));
  Set("MPtBkGEXP",fitMeanpt->GetParameter(16),fitMeanpt->GetParError(16));
...
//  bck->SetParLimits(0, 1.,8.0);
  ...
  fitMeanpt->SetParameter(16, 3.1);
  fitMeanpt->SetParLimits(16, 2.0,5.);
...
//    }
//    else
      fitMeanpt->SetParameter(i + 17, bck->GetParameter(i)*0.95);
  }
  
//  fitMeanpt->SetParLimits(17, -0.5,0.5);
  ...
  Double_t psipPtLim = 10.;
  fitMeanpt->SetParLimits(20, 2.,psipPtLim);
  ...
  Set("MPtBkG0",fitMeanpt->GetParameter(17),fitMeanpt->GetParError(17));
  Set("MPtBkG1",fitMeanpt->GetParameter(18),fitMeanpt->GetParError(18));
  Set("MPtBkG2",fitMeanpt->GetParameter(19),fitMeanpt->GetParError(19));
...
//    fitMeanpt->SetParameter(i + 17, bck->GetParameter(i));
  ...
  Set("MPtBkG1",fitMeanpt->GetParameter(18),fitMeanpt->GetParError(18));
  Set("MPtBkG2",fitMeanpt->GetParameter(19),fitMeanpt->GetParError(19));
  Set("MPtBkGEXP",fitMeanpt->GetParameter(20),fitMeanpt->GetParError(20));
...
//  bck->SetParLimits(0, 1.,8.0);
  ...
  Set("MPtBkG1",fitMeanpt->GetParameter(14),fitMeanpt->GetParError(14));
  Set("MPtBkG2",fitMeanpt->GetParameter(15),fitMeanpt->GetParError(15));
  Set("MPtBkG3",fitMeanpt->GetParameter(16),fitMeanpt->GetParError(16));
...
//_____________________________________________________________________________
void AliAnalysisMuMuJpsiResult::FitMP2TPSIPSIPRIMECB2POL2EXP_BKGMPTPOL2()
{
  //Fit mean dimuon mean pt to get Jpsi mean pt using the CB2 signal parameters, VWG for the Bkg, Jpsi mpt = cte and Bkg mpt = pol2
  fHisto->GetListOfFunctions()->Delete();
...
//  if ( p->GetEntries() < 100000. ) p->Rebin(2);
  ...
  bck->SetParameters(4.,1.,0.);
  
//  bck->SetParLimits(0, 0.,6.0);
  //  bck->SetParLimits(0, 1.,8.0);
  //  TF1* bck = new TF1("bck",this,&AliAnalysisMuMuJpsiResult::FitFunctionBackgroundPol2Exp,fitRangeLow,fitRangeHigh,4,"AliAnalysisMuMuJpsiResult","FitFunctionBackgroundPol2Exp");
  //
  //  bck->SetParameters(1,0,0,0);
  ...
  TF1* fitMeanpt = new TF1("fitMeanpt2",this,&AliAnalysisMuMuJpsiResult::FitFunctionMeanPtS2CB2POL2EXPPOL2,fitRangeLow,fitRangeHigh,17,"AliAnalysisMuMuJpsiResult","FitFunctionMeanPtS2CB2POL2EXPPOL2");
  
  fitMeanpt->SetParNames("pol0","pol1","pol2","exp","kJPsi","mJPsi","sJPsi","alJPsi","nlJPsi","auJPsi","nuJPsi");
  fitMeanpt->SetParName(11,"kPsiP");
  fitMeanpt->SetParName(12,"<pt2>JPsi");
  fitMeanpt->SetParName(13,"<pt2>BG0");
  fitMeanpt->SetParName(14,"<pt2>BG1");
  fitMeanpt->SetParName(15,"<pt2>BG2");
  fitMeanpt->SetParName(16,"<pt2>PsiP");
  ...
  fitMeanpt->SetParameter(12, 9.);
  fitMeanpt->SetParLimits(12, 5.0,12.);
...
//  fitMeanpt->SetParLimits(16, 0.,psipPtLim);
  ...
  Set("MPt2BkG0",fitMeanpt->GetParameter(13),fitMeanpt->GetParError(13));
  Set("MPt2BkG1",fitMeanpt->GetParameter(14),fitMeanpt->GetParError(14));
  Set("MPt2BkG2",fitMeanpt->GetParameter(15),fitMeanpt->GetParError(15));
  ...
  Set("NofJPsi",NofJPsi,ErrStatNofJPsi);
  Set("MeanPt2JPsi",fitMeanpt->GetParameter(12),fitMeanpt->GetParError(12));
  Set("MeanPt2PsiP",fitMeanpt->GetParameter(16),fitMeanpt->GetParError(16));
...
void AliAnalysisMuMuJpsiResult::DecodeFitType(const char* fitType)
{
  ...
  Double_t nofJpsi(0.),nofJpsiErr(0.);
  ...
  while ( ( str = static_cast<TObjString*>(next()) ) )
  {
    ...
    else if ( key.CompareTo("NofJPsi",TString::kIgnoreCase) == 0 )
    {
      nofJpsi =value.Atof();
    }
    else if ( key.CompareTo("ErrStatNofJPsi",TString::kIgnoreCase) == 0 )
    {
      nofJpsiErr =value.Atof();
    }
    else
    {
  ...
  else
  {
    ...
    Set("NofJPsi",nofJpsi,nofJpsiErr);
...
void AliAnalysisMuMuJpsiResult::ProcessMinvFit(TFitResultPtr& fitResult, TF1* fitTotal, TF1* bckInit, const char* fitOption, Int_t iParKPsip, Int_t iLastParBkg)
{
  ...
  if ( static_cast<int>(fitResult) )
  {
    ...
    TFitResultPtr fitResultInit = fHisto->Fit(bckInit,"SRN","",1.7,5.);
...
void AliAnalysisMuMuJpsiResult::ProcessBkgFit(TFitResultPtr &fitResultInit, TF1* bckInit, const char* bkgFuncName, const char* fitOption)
{
  ...
  if ( !sbkgFuncName.CompareTo("FitFunctionBackgroundVWG") )
  {
    ...
    initPars->AddAt(fHisto->GetBinContent(bin)*1.5,0);
    initPars->AddAt(1.,1);
    initPars->AddAt(0.01,2);
    initPars->AddAt(0.03,3);
  }
...
//    initPars->AddAt(0.5,5);
  ...
  if ( static_cast<int>(fitResultInit) )
  {
    ...
    fitResultInit = fHisto->Fit(bckInit,"SRN");
  ...
  if ( static_cast<int>(fitResultInit) )
  {
    ...
    std::cout << "//======== Fitting background again =======\\" << std::endl;
    fitResultInit = fHisto->Fit(bckInit,"SRN","",1.5,5.);
  ...
  if ( static_cast<int>(fitResultInit) )
  {
    ...
    std::cout << "//======== Fitting background again =======\\" << std::endl;
    fitResultInit = fHisto->Fit(bckInit,"SRN","",1.9,4.9);
