  ...
  if ( vertex )
  {
    ...
    if ( SPDVertex )
    {
     ...
     if ( SPDVertex->GetNContributors() > 0 )
     {
       ...
       if ( SPDVertex->IsFromVertexerZ() && (zRes > zResCut) ) return kFALSE;
       
       if ( zDifCut <= 0. ) return kTRUE; // a zDifCut smaller than 0 means not to apply this cut
       else
       {
         if ( TMath::Abs(zvertex - vertex->GetZ()) <= zDifCut )
         {
           return kTRUE;
         }
         else return kFALSE;
...
//_____________________________________________________________________________
Bool_t AliAnalysisMuMuEventCutter::IsNtrCorrInRange(AliVEvent& event, const Double_t& ntrcorrMin, const Double_t& ntrcorrMax) const
{
  Bool_t parFound(kFALSE);
  TParameter<Double_t>* eventdNchdEta;
  ...
  else
  {
    Int_t i(-1);
    Bool_t parFound(kFALSE);
    while ( i < nchList->GetEntries() - 1 && !parFound )
    {
      i++;
  ...
  if ( ntrcorr >= ntrcorrMin && ntrcorr < ntrcorrMax ) return kTRUE;
  else return kFALSE;
...
//_____________________________________________________________________________
void AliAnalysisMuMuEventCutter::NameOfIsNtrCorrInRange(TString& name, const Double_t& ntrCorrMin, const Double_t& ntrCorrMax) const
{
  name.Form("NTRCORRBTW%3.2f_%3.2f",ntrCorrMin,ntrCorrMax);
}
