                                               ...
                                               const char* centrality)
{
  ...
  TObjArray* bins = Binning()->CreateBinObjArray("psi","integrated,ptvsy,yvspt,pt,y,phi,nch,dnchdeta,ntrcorr,ntrcorrpt,ntrcorry,relntrcorr,v0acorr,v0ccorr,v0mcorr","");
    ...
    if ( !IsHistogramDisabled(minvName.Data()) )
    {
      ...
      if ( fcomputeMeanPt )
      {
        ...
        CreatePairHistos(kHistoForData | kHistoForMCInput,eventSelection,triggerClassName,centrality,mPtName.Data(),
                         Form("#mu+#mu- mean p_{T} %s;M_{#mu^{+}#mu^{-}} (GeV/c^2);<p_{T}^{#mu^{+}#mu^{-}}> (GeV/c)",
                              r->AsString().Data()),
        ...
        TString mPt2Name(Form("MeanPt2Vs%s",minvName.Data()));
        CreatePairHistos(kHistoForData | kHistoForMCInput,eventSelection,triggerClassName,centrality,mPt2Name.Data(),
                         Form("#mu+#mu- mean p^{2}_{T} %s;M_{#mu^{+}#mu^{-}} (GeV/c^2);<p_{T}^{#mu^{+}#mu^{-}}^{2}> (GeV^{2}/c^{2})",
                              r->AsString().Data()),
                         nMinvBins,minvMin,minvMax,0);
        
        CreateEventHistos(kHistoForMCInput,eventSelection,triggerClassName,centrality,mPt2Name.Data(),
                          Form("#mu+#mu- mean p^{2}_{T} %s;M_{#mu^{+}#mu^{-}} (GeV/c^2);<p_{T}^{#mu^{+}#mu^{-}}^{2}> (GeV^{2}/c^{2})",
                               r->AsString().Data()),
                          nMinvBins,minvMin,minvMax,0); //Pure MC Histo
        
        CreateEventHistos(kHistoForMCInput,eventSelection,triggerClassName,Form("%s/INYRANGE",centrality),mPt2Name.Data(),
                          Form("#mu+#mu- mean p^{2}_{T} %s;M_{#mu^{+}#mu^{-}} (GeV/c^2);<p_{T}^{#mu^{+}#mu^{-}}^{2}> (GeV^{2}/c^{2})",
                               r->AsString().Data()),
                          nMinvBins,minvMin,minvMax,0); //Pure MC Histo
      }
    ...
    if ( ShouldCorrectDimuonForAccEff() )
    {
      ...
      if ( !IsHistogramDisabled(minvName.Data()) )
      {
        ...
        if ( fcomputeMeanPt )
        {
          TString mPtName(Form("MeanPtVs%s",minvName.Data()));
          CreatePairHistos(kHistoForData | kHistoForMCInput,eventSelection,triggerClassName,centrality,mPtName.Data(),
                           Form("#mu+#mu- mean p_{T} %s (Acc #times Eff Corrected);M_{#mu^{+}#mu^{-}} (GeV/c^{2});<p_{T}^{#mu^{+}#mu^{-}}> (GeV/c)",
                                r->AsString().Data()),
                           nMinvBins,minvMin,minvMax,0);
          
          TString mPt2Name(Form("MeanPt2Vs%s",minvName.Data()));
          CreatePairHistos(kHistoForData | kHistoForMCInput,eventSelection,triggerClassName,centrality,mPt2Name.Data(),
                           Form("#mu+#mu- mean p^{2}_{T} %s (Acc #times Eff Corrected);M_{#mu^{+}#mu^{-}} (GeV/c^{2});<p_{T}^{#mu^{+}#mu^{-}}^{2}> (GeV^{2}/c^{2})",
                                r->AsString().Data()),
                           nMinvBins,minvMin,minvMax,0);
        }
          ...
          if (list)
          {
            ...
            while ( i < list->GetEntries() - 1 && !parFound )
            {
              ...
              if ( TString(p->GetName()).Contains("NtrCorr") )
              {
                ...
                else ok = r->IsInRange(p->GetVal()/5.97); //Outdated and hardcoded, just used to make bins in relative multiplicity
              }
            }
          }
      ...
      if ( ok || okMC )
      {
        ...
        if (!IsHistogramDisabled(minvName.Data()))
        {
          ...
          if ( fcomputeMeanPt )
          {
            ...
            TString hprof2Name(Form("MeanPt2Vs%s",minvName.Data()));
            ...
            if ( ok )
            {
              ...
              TProfile* hprof2 = Prof(eventSelection,triggerClassName,centrality,pairCutName,hprof2Name.Data());
              if ( !hprof2 )
              {
                AliError(Form("Could not get %s",hprof2Name.Data()));
              }
              else
              {
                hprof2->Fill(pair4Momentum.M(),TMath::Power(pair4Momentum.Pt(),2.),inputWeight);
              }
            }
            ...
            if ( okMC )
            {
              ...
              TProfile* hprof2 = MCProf(eventSelection,triggerClassName,centrality,pairCutName,hprof2Name.Data());
              if ( !hprof2 )
              {
                AliError(Form("Could not get MC %s",hprof2Name.Data()));
              }
              else
              {
                hprof2->Fill(pair4MomentumMC->M(),TMath::Power(pair4MomentumMC->Pt(),2.),inputWeightMC);
              }
            }
        ...
        if ( ShouldCorrectDimuonForAccEff() )
        {
          ...
          if (!IsHistogramDisabled(minvName.Data()))
          {
            ...
            if ( fcomputeMeanPt )
            {
              TString hprofCorrName(Form("MeanPtVs%s",minvName.Data()));
              TString hprof2CorrName(Form("MeanPt2Vs%s",minvName.Data()));
              ...
              if( ok )
              {
                ...
                TProfile* hprof2Corr = Prof(eventSelection,triggerClassName,centrality,pairCutName,hprof2CorrName.Data());
                if ( !hprof2Corr )
                {
                  AliError(Form("Could not get %s",hprof2CorrName.Data()));
                }
                else if ( okAccEff )
                {
                  hprof2Corr->Fill(pair4Momentum.M(),TMath::Power(pair4Momentum.Pt(),2.),inputWeight/AccxEff);
                }
              }
              if( okMC )
              {
                ...
                TProfile* hprof2Corr = MCProf(eventSelection,triggerClassName,centrality,pairCutName,hprof2CorrName.Data());
                if ( !hprof2Corr )
                {
                  AliError(Form("Could not get MC %s",hprof2CorrName.Data()));
                }
                else if ( okAccEffMC )
                {
                  hprof2Corr->Fill(pair4MomentumMC->M(),TMath::Power(pair4MomentumMC->Pt(),2.),inputWeightMC/AccxEffMC);
                }
              }
