...
AliAnalysisMuMuNch::~AliAnalysisMuMuNch()
{
  ...
  delete fSPDFluctuationsMapIter;
                                                   ...
                                                   const char* centrality)
{
  ...
  //_____
  CreateEventHistos(kHistoForData | kHistoForMCInput,eventSelection,triggerClassName,centrality,"TrackletsVsZVertexVsEta_Bin0_20","Number of tracklets vs ZVertex vs #eta;ZVertex;#eta",fZAxis->GetNbins(),fZAxis->GetXmin(),fZAxis->GetXmax(),fEtaAxis->GetNbins(),fEtaAxis->GetXmin(),fEtaAxis->GetXmax());
  AttachSPDAcceptance(kHistoForData | kHistoForMCInput,eventSelection,triggerClassName,centrality,"TrackletsVsZVertexVsEta_Bin0_20");
  
  CreateEventHistos(kHistoForData | kHistoForMCInput,eventSelection,triggerClassName,centrality,"TrackletsVsZVertexVsEta_Bin20_60","Number of tracklets vs ZVertex vs #eta;ZVertex;#eta",fZAxis->GetNbins(),fZAxis->GetXmin(),fZAxis->GetXmax(),fEtaAxis->GetNbins(),fEtaAxis->GetXmin(),fEtaAxis->GetXmax());
  AttachSPDAcceptance(kHistoForData | kHistoForMCInput,eventSelection,triggerClassName,centrality,"TrackletsVsZVertexVsEta_Bin20_60");
  
  CreateEventHistos(kHistoForData | kHistoForMCInput,eventSelection,triggerClassName,centrality,"TrackletsVsZVertexVsEta_Bin60_140","Number of tracklets vs ZVertex vs #eta;ZVertex;#eta",fZAxis->GetNbins(),fZAxis->GetXmin(),fZAxis->GetXmax(),fEtaAxis->GetNbins(),fEtaAxis->GetXmin(),fEtaAxis->GetXmax());
  AttachSPDAcceptance(kHistoForData | kHistoForMCInput,eventSelection,triggerClassName,centrality,"TrackletsVsZVertexVsEta_Bin60_140");
  //_____
  ...
  CreateEventHistos(kHistoForMCInput,eventSelection,triggerClassName,centrality,"TrackletsFromPart",Form("Number of tracklets (from gen. part.) in |#eta| < %1.1f distribution;N_{Tracklets};N_{events}",fEtaMax),nbinsMult,multMin,multMax);

  CreateEventHistos(kHistoForData,eventSelection,triggerClassName,centrality,"TrackletsCorrection",Form("Correction for tracklets in |#eta| < %1.1f;Correction;N",fEtaMax),201,-100.5,100.5);
  
  CreateEventHistos(kHistoForData,eventSelection,triggerClassName,centrality,"TrackletsCorrectionVsTracklets",
                    Form("Correction for tracklets in |#eta| < %1.1f vs tracklets;N_{tr};Correction",fEtaMax),nbinsMult,multMin,multMax,201,-100.5,100.5);
  ...
  CreateEventHistos(kHistoForMCInput,eventSelection,triggerClassName,centrality,"TrackletsFromPartVsZVertexVsEta","Number of trkl. from a gen part. vs ZVertex vs #eta;ZVertex;#eta",fZAxis->GetNbins(),fZAxis->GetXmin(),fZAxis->GetXmax(),fEtaAxis->GetNbins(),fEtaAxis->GetXmin(),fEtaAxis->GetXmax());
  AttachSPDAcceptance(kHistoForMCInput,eventSelection,triggerClassName,centrality,"TrackletsFromPartVsZVertexVsEta");
  ...
  CreateEventHistos(kHistoForMCInput,eventSelection,triggerClassName,centrality,"NchVsZVertexVsEta_Bin0_20","Number of charged particles vs ZVertex vs #eta;ZVertex;#eta",fZAxis->GetNbins(),fZAxis->GetXmin(),fZAxis->GetXmax(),fEtaAxis->GetNbins(),fEtaAxis->GetXmin(),fEtaAxis->GetXmax());
  
  CreateEventHistos(kHistoForMCInput,eventSelection,triggerClassName,centrality,"NchVsZVertexVsEta_Bin20_60","Number of charged particles vs ZVertex vs #eta;ZVertex;#eta",fZAxis->GetNbins(),fZAxis->GetXmin(),fZAxis->GetXmax(),fEtaAxis->GetNbins(),fEtaAxis->GetXmin(),fEtaAxis->GetXmax());
  
  CreateEventHistos(kHistoForMCInput,eventSelection,triggerClassName,centrality,"NchVsZVertexVsEta_Bin60_140","Number of charged particles vs ZVertex vs #eta;ZVertex;#eta",fZAxis->GetNbins(),fZAxis->GetXmin(),fZAxis->GetXmax(),fEtaAxis->GetNbins(),fEtaAxis->GetXmin(),fEtaAxis->GetXmax());
...
//  CreateEventHistos(kHistoForMCInput,eventSelection,triggerClassName,centrality,"NchVsTracklets","Number of generated charged particles vs reco tracklets;N_{tracklets};N_{ch}",nbinsMult,multMin,multMax,nbinsMult,multMin,multMax);
  ...
  CreateEventHistos(kHistoForMCInput,eventSelection,triggerClassName,centrality,"MeanTrackletsFromPartVsZVertex",Form("Mean number of tracklets (from gen. part.) in |#eta| < %1.1f vs Z vertex;Z vertex;<N_{Tracklets}>",fEtaMax),fZAxis->GetNbins(),fZAxis->GetXmin(),fZAxis->GetXmax(),0);
...
void AliAnalysisMuMuNch::DefineSPDCorrectionMap(TObjArray* spdCorrectionList)
{
  ...
  {
    if ( static_cast<TH2*>(SPDCorrection)->IsA() != TH2F::Class() &&  static_cast<TH2*>(SPDCorrection)->IsA() != TH2D::Class() && static_cast<TH1*>(SPDCorrection)->IsA() != TProfile::Class() )
    {
      AliFatal(Form("Unrecognized SPD correction Class %s",static_cast<TH2*>(SPDCorrection)->IsA()->GetName()));
    }
...
void AliAnalysisMuMuNch::DefineSPDFluctuationsMap(TH2F* spdFluctuations)
{
  ...
  if ( !fSPDFluctuationsMap ) // Creates a TMap for the run->Correction correspondance
  {
    ...
    fSPDFluctuationsMapIter = new TMapIter(fSPDFluctuationsMap);
  ...
  for ( Int_t i = 1 ; i <= 70 ; i++ ) // Bins 1 to 70; In steps of 1 bin
  {
  ...
  for ( Int_t i = 0 ; i < 5 ; i++ ) // Bins 71 to 80; In steps of 2 bin
  {
    Int_t bin = 71 + 2*i;
    ...
    h->SetDirectory(0);
    fSPDFluctuationsList->Add(h);
    fSPDFluctuationsMap->Add(new TObjString(Form("%f.2-%f.2",h->GetXaxis()->GetBinLowEdge(bin),h->GetXaxis()->GetBinUpEdge(bin+1))),new TObjString(Form("%d",70+i))); // Map between Ntr corrected by AccxEff and fluctuations distribution index in fSPDFluctuationsList
  }
  
  TH1D* h10 = spdFluctuations->ProjectionX("Fluctuations_81_150",81,150);
  h10->SetDirectory(0);
  fSPDFluctuationsList->Add(h10); // Add fluctuations distribution to array
  fSPDFluctuationsMap->Add(new TObjString("80.5-149.5"),new TObjString("75")); // Map between Ntr corrected by AccxEff and fluctuations distribution index in fSPDFluctuationsList
}
...
Double_t AliAnalysisMuMuNch::ApplyFluctuations(Double_t dNtrCorrdeta)
{
  ...
  Int_t iSPDFluctuationsIndex(-1);
  Int_t i(0);
  Bool_t foundRange(kFALSE);
  
  //  while ( i <= 70 && !foundRange )
  //  {
  ...
  //  if ( !foundRange )
  //  {
  //    if ( (dNtrCorrdeta >= 80.5) && (dNtrCorrdeta < 149.5) ) iSPDFluctuationsIndex = 75;
  //  }
  
  fSPDFluctuationsMapIter->Reset();
  TObjString* keyName(0x0);
  
  while ( (keyName = static_cast<TObjString*>(fSPDFluctuationsMapIter->Next())) )
  {
    TString sl = keyName->GetString();
    TString su = keyName->GetString();
    
    sl.Remove(sl.First("-"),sl.Sizeof());
    su.Remove(0,su.First("-")+1);
    
    Double_t lowE = sl.Atof();
    Double_t upE = su.Atof();
    
    if ( (dNtrCorrdeta >= lowE) && (dNtrCorrdeta<upE) )
    {
      TObjString* mapVal = static_cast<TObjString*>(fSPDFluctuationsMap->GetValue(keyName));
      iSPDFluctuationsIndex = mapVal->GetString().Atof();
    }
                                            ...
                                            const char* centrality)
{
  ...
  //____ Get the NtrCorr or Nch values from the list (if any)
  Int_t i(-1);
  ...
  while ( i < nchList->GetEntries() - 1 && !(parFound1 && parFound2) )
  {
    ...
    if ( TString(p->GetName()).Contains("NtrCorr") )
    {
      ...
      if ( NtrCorr == -1 ) return; // We do not fill the histos if the NtrCorr has been set as invalid
    }
    else if ( TString(p->GetName()).Contains("MeandNchdEta") )
    {
      ...
      if ( dNchdeta == -1 ) return; // We do not fill the histos if the dNchdeta has been set as invalid
    }
  }
  //______
  ...
  for (Int_t j = etaBinMin[0] ; j <= etaBinMax[0] ; j++) // Loop over eta bins.
  {
    ...
    else hNchVsEta->Reset(); // If there are no SPD corection we reset the 2D Nch histo
  ...
  {
    Double_t SPDr = NtrCorr - nTracklets[0]; // Get 'mean correction' for the zvtx w/o generating a new random
    
    Histo(eventSelection,triggerClassName,centrality,"TrackletsCorrection")->Fill(SPDr); //Control plots for the correction
    Histo(eventSelection,triggerClassName,centrality,"TrackletsCorrectionVsTracklets")->Fill(nTracklets[0],SPDr);
  ...
  else if ( fSPDMeanTracklets && fSPDOneOverAccxEff ) // Here we compare the Corrected tracklets with the two correction methods. Just used for testing
  {
...
//      Double_t dNchdetaPubli = 17.35; //FIXME: hardcoded (pPb value)
  ...
  if ( nTracklets[0] >= 0 && nTracklets[0] <= 20) AddHisto(eventSelection,triggerClassName,centrality,"TrackletsVsZVertexVsEta_Bin0_20",SPDZv,hNTrackletVsEta);
  else if ( nTracklets[0] > 20 && nTracklets[0] <= 60) AddHisto(eventSelection,triggerClassName,centrality,"TrackletsVsZVertexVsEta_Bin20_60",SPDZv,hNTrackletVsEta);
  else if ( nTracklets[0] > 60 && nTracklets[0] <= 140) AddHisto(eventSelection,triggerClassName,centrality,"TrackletsVsZVertexVsEta_Bin60_140",SPDZv,hNTrackletVsEta);
...
//  Int_t i(-1);
  ...
  if ( nBins >  0 )
  {
    if ( fSPDOneOverAccxEff )
    {
      ...
      {
        meandNchdEta = ApplyFluctuations(meandNchdEta); //FIXME: Due to the TRandom this is not the same in SetEvent() and here
      }
    }
...
void AliAnalysisMuMuNch::FillHistosForMCEvent(const char* eventSelection,const char* triggerClassName,const char* centrality)
{
  ...
  TH1* hNTrackletFromPartVsEta = static_cast<TH1*>(nchList->FindObject("MCNTrackletFromPartVsEta"));
  ...
  Double_t nchSum(0.),nTracklets(0.),nTrackletsFromPart(0.);
  for (Int_t j = 1 ; j <= fEtaAxis->GetNbins() ; j++) //Loop over all eta bins 
  {
    ...
    nTrackletsFromPart += hNTrackletFromPartVsEta->GetBinContent(j); // Reco tracklets coming from a generated particle
  ...
  MCHisto(eventSelection,triggerClassName,centrality,"TrackletsFromPart")->Fill(nTrackletsFromPart,fMCWeight);
  static_cast<TProfile*>(MCHisto(eventSelection,triggerClassName,centrality,"MeanTrackletsFromPartVsZVertex"))->Fill(SPDZv,nTrackletsFromPart,fMCWeight);
  ...
  AddHisto(eventSelection,triggerClassName,centrality,"TrackletsFromPartVsZVertexVsEta",SPDZv,hNTrackletFromPartVsEta,isMChisto);
...
//      }
      Double_t Ntr = p->GetVal();
      if ( Ntr >= 0 && Ntr <= 20) AddHisto(eventSelection,triggerClassName,centrality,"NchVsZVertexVsEta_Bin0_20",MCZv,hNchVsEta,isMChisto);
      else if ( Ntr > 20 && Ntr <= 60) AddHisto(eventSelection,triggerClassName,centrality,"NchVsZVertexVsEta_Bin20_60",MCZv,hNchVsEta,isMChisto);
      else if ( Ntr > 60 && Ntr <= 140) AddHisto(eventSelection,triggerClassName,centrality,"NchVsZVertexVsEta_Bin60_140",MCZv,hNchVsEta,isMChisto);
...
//      else if ( nTracklets*mNTtklsZref/mNTtklsZ > 69.5 ) meanPoiss = meanPoiss*0.91;
      ...
      if ( (coef == -1) && (TMath::Abs(deltaN) >= nTracklets) ) deltaN = coef*nTracklets; // To avoid negative tracklets after correction
...
void AliAnalysisMuMuNch::SetEvent(AliVEvent* event, AliMCEvent* mcEvent)
{
  ...
  TH1* hNTrackletFromPartVsEta(0x0);
  ...
  if ( !fResolution ) // When computing resolutions we dont do anything else
  {
    ...
    if ( !Histo("AliAnalysisMuMuNch","SPDcorrectionVsEta") )
    {
      ...
      CreateEventHistos(kHistoForMCInput,"AliAnalysisMuMuNch","NTrackletFromPartVsEta","Ntracklet comming from gen. particle vs #eta;#eta",fEtaAxis->GetNbins(),fEtaAxis->GetXmin(),fEtaAxis->GetXmax());
    ...
    if ( tracklets && isEtaRangeValid )
    {
      ...
      if ( fSPDOneOverAccxEff || fSPDMeanTracklets ) // Here we correct the raw tracklets
      {
        if ( fSPDOneOverAccxEff ) // In this case the correction is the SPD accxEff
        {
          ...
          nchList->Add(new TParameter<Double_t>("MeandNchdEta",meandNchdEta)); // We add the mean(eta averaged) dNch/dEta to the event. It will serve us as a multiplicity estimator.
        }

        if( fSPDMeanTracklets ) // In this case the correction is the 'mean correction'
        {
//          if ( HasMC() ) //Test for the correction of events not passing QA using the MC vertex
//          {
//            const AliVVertex* vertex = event->GetPrimaryVertex();
//            
//            TString vtxTyp = vertexSPD->GetTitle();
//            if ( vertexSPD->GetNContributors() > 0 && !vtxTyp.Contains("vertexer: Z") )
//            {
//              Double_t cov[6]={0};
//              vertexSPD->GetCovarianceMatrix(cov);
//              Double_t zRes = TMath::Sqrt(cov[5]);
//              Double_t zvertex = vertexSPD->GetZ();
//              
//              if ( (zRes <= 0.25) && TMath::Abs(zvertex - vertex->GetZ()) <= 0.5 )
//              {
//                SPDZv = SPDZv; //Do nothing with the vertex
//              }
//              else SPDr = AliAnalysisMuonUtility::GetMCVertexZ(Event(),MCEvent()); // If the event does not pass QA MC vertex is used to correct multiplicity
//            }
//            else SPDr = AliAnalysisMuonUtility::GetMCVertexZ(Event(),MCEvent());
//          }
  ...
  if ( HasMC() )
  {
    ...
    if ( !fResolution ) //When computing resolutions we dont do anything else
    {
      ...
      hNTrackletFromPartVsEta = MCHisto("AliAnalysisMuMuNch","NTrackletFromPartVsEta");
      hNTrackletVsPhi = MCHisto("AliAnalysisMuMuNch","NTrackletVsPhi");
      
      hNTrackletVsEta->Reset();
      hNTrackletFromPartVsEta->Reset();
      hNchVsEta->Reset();
      hNchVsPhi->Reset();
...
//        if ( !IsMCtrackFromGenerator(label1) || !IsMCtrackFromGenerator(label2) ) continue; // Select only the particles generated by the desired generator
        ...
        if ( !fResolution )
        {
          ...
          if ( label1 == label2 ) hNTrackletFromPartVsEta->Fill(etaTracklet);
        }
      ...
      if (!fResolution )
      {
        ...
        nchList->Add(hNTrackletFromPartVsEta->Clone("MCNTrackletFromPartVsEta"));
...
void AliAnalysisMuMuNch::SetRun(const AliInputEventHandler* eventHandler)
{
  ...
  if ( fSPDCorrectionList )
  {
    ...
    if ( static_cast<TH1*>(hSPDCorrection)->IsA() == TH2F::Class() ) fSPDOneOverAccxEff = static_cast<TH2F*>(hSPDCorrection); // Sets the corresponding correction data member
