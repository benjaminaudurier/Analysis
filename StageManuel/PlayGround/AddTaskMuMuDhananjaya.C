                             ...
                             Bool_t simulations = kFALSE)
{
  ...
  /* TProfile* hT =0x0;
  const char * filename1 ="AnalysisResults15l.root";
  AliAnalysisMuMu ana1(filename1,"","","mumu.pp2015.config");
  hT = static_cast<TProfile*>(ana1.OC()->GetObject("/PSMULHASSPDSPDZQA_RES0.25_ZDIF0.50SPDABSZLT10.00TZEROPILEUP/CMUL7-B-NOPF-MUFAST/PP/MeanTrackletsVsZVertex")->Clone()); */
  ...
  /* TFile* f =TFile::Open("test_distribution.root");
   ...
   //TList *l = (TList*) f->Get("OC/PSMULHASSPDSPDZQA_RES0.25_zdif0.50spdabszlt10.00/CMUL7-B-NOPF-MUFAST/V0A");
     TProfile* hT =0x0;
     f->GetObject("OC/PSMULHASSPDSPDZQA_RES0.25_ZDIF0.50SPDABSZLT10.00/CMUL7-B-NOPF-MUFAST/V0A/MeanTrackletsVsZVertex",hT);
     if(!hT){
  ...
  // AliAnalysisMuMuNch* nch              = new AliAnalysisMuMuNch(0x0,hT,22.76,etaMin,etaMax,zMin,zMax,kFALSE);//Raw Tracklets Analysis
  ...
  AliAnalysisMuMuCutElement* cutSPDZ_Minus10_Minus5     = cr->AddEventCut(*eventCutter,"IsSPDzVertexInRange","const AliVEvent&,Double_t,Double_t","-10.0 , -5.0");
  AliAnalysisMuMuCutElement* cutSPDZ_Minus5_0           = cr->AddEventCut(*eventCutter,"IsSPDzVertexInRange","const AliVEvent&,Double_t,Double_t","-5.0 , 0.0");
  AliAnalysisMuMuCutElement* cutSPDZ_0_5                = cr->AddEventCut(*eventCutter,"IsSPDzVertexInRange","const AliVEvent&,Double_t,Double_t","0.0 , 5.0");
  AliAnalysisMuMuCutElement* cutSPDZ_5_10               = cr->AddEventCut(*eventCutter,"IsSPDzVertexInRange","const AliVEvent&,Double_t,Double_t"," 5.0 , 10.0");
  ...
  cr->AddCutCombination(ps1,cutHasSPD,cutZQA,cutSPDZ10,triggerSelection,NoSPDPileUp);

  // cr->AddCutCombination(ps1,cutHasSPD,cutZQA,cutSPDZ_Minus10_Minus5,triggerSelection);
  // cr->AddCutCombination(ps1,cutHasSPD,cutZQA,cutSPDZ_Minus10_Minus5,triggerSelection,NoSPDPileUp);

  // cr->AddCutCombination(ps1,cutHasSPD,cutZQA,cutSPDZ_Minus5_0,triggerSelection);
  // cr->AddCutCombination(ps1,cutHasSPD,cutZQA,cutSPDZ_Minus5_0,triggerSelection,NoSPDPileUp);

  // cr->AddCutCombination(ps1,cutHasSPD,cutZQA,cutSPDZ_0_5,triggerSelection);
  // cr->AddCutCombination(ps1,cutHasSPD,cutZQA,cutSPDZ_0_5,triggerSelection,NoSPDPileUp);

  // cr->AddCutCombination(ps1,cutHasSPD,cutZQA,cutSPDZ_5_10,triggerSelection);
  // cr->AddCutCombination(ps1,cutHasSPD,cutZQA,cutSPDZ_5_10,triggerSelection,NoSPDPileUp);
