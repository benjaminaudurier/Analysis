...
///
/// Class to equalize the LHC15o ZDC Q Vector
///
...
void AliAnalysisTaskZDCEP::UserCreateOutputObjects ()
{
  ...
  // Lists of QA histo for the equalization proceedures
  fOutputHistoList = new TList();
  fOutputHistoList->SetOwner(kTRUE);

  // Equalization histo
  TH1* h_Eq_ZDCA_Step0 = new TH1F("Eq_ZDCAEventPlane_step0","ZDCA Event Plane Distribution (step 0)",600, -1.6, 1.6);
  TH1* h_Eq_ZDCA_Step1 = new TH1F("Eq_ZDCAEventPlane_step1","ZDCA Event Plane Distribution (step 1)",600, -1.6, 1.6);
  TH1* h_Eq_ZDCA_Step2 = new TH1F("Eq_ZDCAEventPlane_step2","ZDCA Event Plane Distribution (step 2)",600, -1.6, 1.6);
  TH1* h_Eq_ZDCA_Step3 = new TH1F("Eq_ZDCAEventPlane_step3","ZDCA Event Plane Distribution (step 3)",600, -1.6, 1.6);
  TH1* h_Eq_ZDCA_Step4 = new TH1F("Eq_ZDCAEventPlane_step4","ZDCA Event Plane Distribution (step 4)",600, -1.6, 1.6);

  TH1* h_Eq_ZDCC_Step0 = new TH1F("Eq_ZDCCEventPlane_step0","ZDCC Event Plane Distribution (step 0)",600, -1.6, 1.6);
  TH1* h_Eq_ZDCC_Step1 = new TH1F("Eq_ZDCCEventPlane_step1","ZDCC Event Plane Distribution (step 1)",600, -1.6, 1.6);
  TH1* h_Eq_ZDCC_Step2 = new TH1F("Eq_ZDCCEventPlane_step2","ZDCC Event Plane Distribution (step 2)",600, -1.6, 1.6);
  TH1* h_Eq_ZDCC_Step3 = new TH1F("Eq_ZDCCEventPlane_step3","ZDCC Event Plane Distribution (step 3)",600, -1.6, 1.6);
  TH1* h_Eq_ZDCC_Step4 = new TH1F("Eq_ZDCCEventPlane_step4","ZDCC Event Plane Distribution (step 4)",600, -1.6, 1.6);

  fOutputHistoList->Add(h_Eq_ZDCA_Step0);
  fOutputHistoList->Add(h_Eq_ZDCA_Step1);
  fOutputHistoList->Add(h_Eq_ZDCA_Step2);
  fOutputHistoList->Add(h_Eq_ZDCA_Step3);
  fOutputHistoList->Add(h_Eq_ZDCA_Step4);
  fOutputHistoList->Add(h_Eq_ZDCC_Step0);
  fOutputHistoList->Add(h_Eq_ZDCC_Step1);
  fOutputHistoList->Add(h_Eq_ZDCC_Step2);
  fOutputHistoList->Add(h_Eq_ZDCC_Step3);
  fOutputHistoList->Add(h_Eq_ZDCC_Step4);

  // Correlation angle ZDCA and ZDCC histo
  TH2* h_PhiCVsPhiA_Step0 = new TH2F("PhiCVsPhiA_step0","ZDCA vs ZDCC Event Plane (step 0);ZDC_{A};ZDC_{C}",600, -1.6, 1.6,600, -1.6, 1.6);
  TH2* h_PhiCVsPhiA_Step1 = new TH2F("PhiCVsPhiA_step1","ZDCA vs ZDCC Event Plane (step 1);ZDC_{A};ZDC_{C}",600, -1.6, 1.6,600, -1.6, 1.6);
  TH2* h_PhiCVsPhiA_Step2 = new TH2F("PhiCVsPhiA_step2","ZDCA vs ZDCC Event Plane (step 2);ZDC_{A};ZDC_{C}",600, -1.6, 1.6,600, -1.6, 1.6);
  TH2* h_PhiCVsPhiA_Step3 = new TH2F("PhiCVsPhiA_step3","ZDCA vs ZDCC Event Plane (step 3);ZDC_{A};ZDC_{C}",600, -1.6, 1.6,600, -1.6, 1.6);
  TH2* h_PhiCVsPhiA_Step4 = new TH2F("PhiCVsPhiA_step4","ZDCA vs ZDCC Event Plane (step 4);ZDC_{A};ZDC_{C}",600, -1.6, 1.6,600, -1.6, 1.6);

  fOutputHistoList->Add(h_PhiCVsPhiA_Step0);
  fOutputHistoList->Add(h_PhiCVsPhiA_Step1);
  fOutputHistoList->Add(h_PhiCVsPhiA_Step2);
  fOutputHistoList->Add(h_PhiCVsPhiA_Step3);
  fOutputHistoList->Add(h_PhiCVsPhiA_Step4);

  // Correlation and anti-correlation Q vectors after equalization
  TH2* h_QxAQxC = new TH2F("QxAvsQxC","QxA X QxC versus centrality;centrality;QxA X QxC",9, 0, 90,600, -1.6, 1.6);
  TH2* h_QyAQyC = new TH2F("QyAvsQxC","QyA X QyC versus centrality;centrality;QyA X QyC",9, 0, 90,600, -1.6, 1.6);
  TH2* h_QxAQyC = new TH2F("QxAvsQyC","QxA X QyC versus centrality;centrality;QxA X QyC",9, 0, 90,600, -1.6, 1.6);
  TH2* h_QyAQxC = new TH2F("QyAvsQxC","QyA X QxC versus centrality;centrality;QyA X QxC",9, 0, 90,600, -1.6, 1.6);

  fOutputHistoList->Add(h_QxAQxC);
  fOutputHistoList->Add(h_QyAQyC);
  fOutputHistoList->Add(h_QxAQyC);
  fOutputHistoList->Add(h_QyAQxC);
...
void AliAnalysisTaskZDCEP::UserExec(Option_t *)
{
  ...
  // Get Centrality and event run number
  ...
  if(fZDCCalibList) {
    ...
    {
      // get histos of run
      if(fZDCCalibList->FindObject(Form("Run %d",RunNum))->FindObject(Form("fCRCZDCQVecC[%d][%d]",RunNum,0)))
        fZDCQHist[0] = (TProfile*)(fZDCCalibList->FindObject(Form("Run %d",RunNum))->FindObject(Form("fCRCZDCQVecC[%d][%d]",RunNum,0)));
      else {
        printf("cannot find fCRCZDCQVecC[%d][%d]\n",RunNum,0);
        return;
      }

      if(fZDCCalibList->FindObject(Form("Run %d",RunNum))->FindObject(Form("fCRCZDCQVecC[%d][%d]",RunNum,1)))
      ...
      if(fZDCCalibList->FindObject(Form("Run %d",RunNum))->FindObject(Form("fCRCZDCQVecA[%d][%d]",RunNum,0)))
        fZDCQHist[2] = (TProfile*)(fZDCCalibList->FindObject(Form("Run %d",RunNum))->FindObject(Form("fCRCZDCQVecA[%d][%d]",RunNum,0)));
      else {
        printf("cannot find fCRCZDCQVecA[%d][%d]\n",RunNum,0);
        return;
      }

      if(fZDCCalibList->FindObject(Form("Run %d",RunNum))->FindObject(Form("fCRCZDCQVecA[%d][%d]",RunNum,1)))
    ...
    static_cast<TH1*>(fOutputHistoList->At(0))->Fill(GetEventPlaneV1(QARe,QAIm));
    static_cast<TH1*>(fOutputHistoList->At(4))->Fill(GetEventPlaneV1(QCRe,QCIm));
    static_cast<TH2*>(fOutputHistoList->At(10))->Fill(GetEventPlaneV1(QARe,QAIm),GetEventPlaneV1(QCRe,QCIm));
    ...
    if (fZDCQHist[0]) {
      ...
      static_cast<TH1*>(fOutputHistoList->At(1))->Fill(GetEventPlaneV1(QAReR,QAImR));
      static_cast<TH1*>(fOutputHistoList->At(5))->Fill(GetEventPlaneV1(QCReR,QCImR));
      static_cast<TH2*>(fOutputHistoList->At(11))->Fill(GetEventPlaneV1(QARe,QAIm),GetEventPlaneV1(QCRe,QCIm));
    }
    ...
    if (fZDCVtxCenHist[fCenBin][0]) {
      ...
      static_cast<TH1*>(fOutputHistoList->At(2))->Fill(GetEventPlaneV1(QAReR,QAImR));
      static_cast<TH1*>(fOutputHistoList->At(6))->Fill(GetEventPlaneV1(QCReR,QCImR));
      static_cast<TH2*>(fOutputHistoList->At(12))->Fill(GetEventPlaneV1(QARe,QAIm),GetEventPlaneV1(QCRe,QCIm));
    }
    ...
    if (fZDCVtxHist[0]) {
      ...
      static_cast<TH1*>(fOutputHistoList->At(3))->Fill(GetEventPlaneV1(QAReR,QAImR));
      static_cast<TH1*>(fOutputHistoList->At(7))->Fill(GetEventPlaneV1(QCReR,QCImR));
      static_cast<TH2*>(fOutputHistoList->At(13))->Fill(GetEventPlaneV1(QARe,QAIm),GetEventPlaneV1(QCRe,QCIm));
    }
    ...
    if(fZDCEcomTotHist[0]) {
      ...
      static_cast<TH1*>(fOutputHistoList->At(4))->Fill(GetEventPlaneV1(QAReR,QAImR));
      static_cast<TH1*>(fOutputHistoList->At(8))->Fill(GetEventPlaneV1(QCReR,QCImR));
      static_cast<TH2*>(fOutputHistoList->At(14))->Fill(GetEventPlaneV1(QARe,QAIm),GetEventPlaneV1(QCRe,QCIm));

      static_cast<TH2*>(fOutputHistoList->At(15))->Fill(Centrality,QAReR*QCReR);
      static_cast<TH2*>(fOutputHistoList->At(16))->Fill(Centrality,QAImR*QCImR);
      static_cast<TH2*>(fOutputHistoList->At(17))->Fill(Centrality,QAReR*QCImR);
      static_cast<TH2*>(fOutputHistoList->At(18))->Fill(Centrality,QAImR*QCReR);
...
//_____________________________________________________________________________
Double_t AliAnalysisTaskZDCEP::GetEventPlaneV1(Double_t QX, Double_t QY)
{
  if(TMath::Abs(QX) < 1e-6 && TMath::Abs(QY) < 1e-6) {
    return 0.0;
  }
