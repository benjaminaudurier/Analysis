                                               ...
                                               const char* centrality)
{
  ...
  while ( ( r = static_cast<AliAnalysisMuMuBinning::Range*>(next()) ) ){
    // Histos name
    TString minvName(GetMinvHistoName(*r,kFALSE,0));
    TString minvNamePlus(GetMinvHistoName(*r,kFALSE,2));
    TString minvNameMinus(GetMinvHistoName(*r,kFALSE,-2));
    ++nb;
    ...
    {
      minvName      = GetMinvHistoName(*r,kTRUE,0);
      minvNamePlus  = GetMinvHistoName(*r,kTRUE,2);
      minvNameMinus = GetMinvHistoName(*r,kTRUE,-2);
                                            ...
                                            const Bool_t IsMixedHisto)
{
  ...
  // Usual cuts
  if ( ( tracki.Charge() == trackj.Charge() && !IsMixedHisto ) ) return;
  ...
  while ( ( r = static_cast<AliAnalysisMuMuBinning::Range*>(nextBin()) ) ){
    ...
    if ( ok || okMC ){
      ...
      TString minvName = GetMinvHistoName(*r,kFALSE,PairCharge);
      ...
      if ( ShouldCorrectDimuonForAccEff() ){
        ...
        // Get histo name
        minvName = GetMinvHistoName(*r,kTRUE,PairCharge);
...
//_____________________________________________________________________________
TString AliAnalysisMuMuMinv::GetMinvHistoName(const AliAnalysisMuMuBinning::Range& r, Bool_t accEffCorrected, Double_t PairCharge) const
{
  TString suffix;
