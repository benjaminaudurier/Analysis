...
public:
  ...
  virtual void UserExecMix(Option_t* opt);
...
private:
  ...
  AliVEvent* MixedEvent(Int_t buffId) const;
  
  void FillHistos(const char* eventSelection, const char* triggerClassName, const char* centrality, Int_t buffId);
  
  void FillCounters(const char* eventSelection, const char* triggerClassName, const char* centrality, Int_t currentRun);
  
  void Fill(const char* eventSelection, const char* triggerClassName, Int_t buffId =0);
...
private:
  ...
  ClassDef(AliAnalysisTaskMuMu,29) // a class to analyse muon pairs (and single also ;-) )
};
