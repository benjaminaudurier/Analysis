...
//_____________________________________________________________________________
AliVEvent*
AliAnalysisTaskMuMu::MixedEvent(Int_t buffId) const
{
   //
...
//_____________________________________________________________________________
void AliAnalysisTaskMuMu::Fill(const char* eventSelection, const char* triggerClassName, Int_t buffId)
{
  ...
  while ( ( r = static_cast<AliAnalysisMuMuBinning::Range*>(next()) ) ){
    ...
    if ( isPP || r->IsInRange(fcent) ){
      FillHistos(eventSelection,triggerClassName,r->AsString(),buffId);
                                     ...
                                     Int_t buffId)
{
  ...
  Int_t nTrackMix = (!fMix) ? AliAnalysisMuonUtility::GetNTracks(Event()) : AliAnalysisMuonUtility::GetNTracks(MixedEvent(buffId));
    ...
    while ( ( analysis = static_cast<AliAnalysisMuMuBase*>(nextAnalysis()) ) ){
      ...
      for (Int_t i = 0; i < nTracks; ++i){
       ...
       if (!AliAnalysisMuonUtility::IsMuonTrack(tracki) ) continue;
        ...
        for (Int_t j = i+1; j < nTrackMix; ++j){
          // Get track
          AliVParticle* trackj = 0x0;
          if(!fMix)trackj = AliAnalysisMuonUtility::GetTrack(j,Event());
          else trackj = AliAnalysisMuonUtility::GetTrack(j,MixedEvent(buffId));
          ...
          while ( ( pairCut = static_cast<AliAnalysisMuMuCutCombination*>(nextPairCut()) ) ){
            ...
            if ( ( testi && testj ) && testij ){
              ...
              analysis->FillHistosForPair(eventSelection,triggerClassName,centrality,pairCut->GetName(),*tracki,*trackj,fMix);
            }
          }
        }
      }
...
//_____________________________________________________________________________
void AliAnalysisTaskMuMu::UserExecMix(Option_t* /*opt*/)
{
  /// Executed at each event
...
//
  ...
  for(Int_t iBuffer=0; iBuffer<bufferSize; iBuffer++) {
    ...
    while ( ( analysis = static_cast<AliAnalysisMuMuBase*>(nextAnalysis()) ) ) {
      ...
      // MC and real trigger classes)
      if ( MCEvent() ) analysis->SetMC();
      analysis->SetEvent(MixedEvent(iBuffer),MCEvent()); // Set the new event properties derived in the analysis
    }


    TString firedTriggerClasses(MixedEvent(iBuffer)->GetFiredTriggerClasses());
    ...
    GetSelectedTrigClassesInEvent(MixedEvent(iBuffer),selectedTriggerClasses);
    ...
    while ( ( tname = static_cast<TObjString*>(next()) ) ){
      ...
      while ( ( cutCombination = static_cast<AliAnalysisMuMuCutCombination*>(nextEventCutCombination())) ){
        if ( cutCombination->Pass(*fInputHandler) ) Fill(cutCombination->GetName(),tname->String().Data(),iBuffer);
      }
    }
  }
  // Post output data.
...
AliMultiInputEventHandler *AliAnalysisTaskMuMu::SetMainInputHandler(AliAnalysisManager *mgr)
{
   //
...
//________________________________________________________________________
AliMixInputEventHandler *AliAnalysisTaskMuMu::SetMixingInputHandler(AliMultiInputEventHandler *mainIH)
{
   //
