//
// Class AliMixInfosTaskMixInfo2
//
// AliAnalysisTaskMixInfo2 is task
// for mixing info
//
// Mixing info can be enabled by setting one of following lines in UserCreateOutput() in your task
//       // prints mixing info
//       AliLog::SetClassDebugLevel("AliAnalysisTaskMixInfo2", AliLog::kDebug);
//       // prints mixing info + event info for both (main and mixed) events
//       AliLog::SetClassDebugLevel("AliAnalysisTaskMixInfo2", AliLog::kDebug+1);
//
// authors:
...
#include "AliAnalysisTaskMixInfo2.h"
#include "AliMixInfo2.h"
#include "AliMixEventPool2.h"
#include "AliMixEventCutObj2.h"


ClassImp(AliAnalysisTaskMixInfo2)

//________________________________________________________________________
AliAnalysisTaskMixInfo2::AliAnalysisTaskMixInfo2(const char *name)
...
//________________________________________________________________________
AliAnalysisTaskMixInfo2::~AliAnalysisTaskMixInfo2()
{
   //
   // Destructor
   //
...
//________________________________________________________________________
void AliAnalysisTaskMixInfo2::UserCreateOutputObjects()
{
   // Create histograms
   // Called once
   ...
   if (fInputEHMix) {
      AliMixEventPool2 *evPool = fInputEHMix->GetEventPool();
      if (evPool) {
...
//________________________________________________________________________
void AliAnalysisTaskMixInfo2::UserExec(Option_t *)
{
   // Main loop
   ...
   if (fMixInfo && fInputEHMix) {
      if (fInputEHMix->BufferSize() > 1) {
         if (fInputEHMix->NumberMixedTimes() >= fInputEHMix->BufferSize())
            fMixInfo->FillHistogram(AliMixInfo2::kMainEvents, fInputEHMix->CurrentBinIndex());
      } else {
...
//________________________________________________________________________
void AliAnalysisTaskMixInfo2::UserExecMix(Option_t *)
{
   // UserExecMix
   ...
   if (fMixInfo) {
      if (fInputEHMix->CurrentBinIndex()==-1) {
         fMixInfo->FillHistogram(AliMixInfo2::kMixedEvents, fInputEHMix->CurrentBinIndex());
      } else {
...
//________________________________________________________________________
void AliAnalysisTaskMixInfo2::FinishTaskOutput()
{
   // FinishTaskOutput
...
//________________________________________________________________________
void AliAnalysisTaskMixInfo2::Terminate(Option_t *)
{
   ...
   fOutputList->Print();
   fMixInfo = (AliMixInfo2 *) fOutputList->FindObject("mixInfo");
   if (fMixInfo) {
      fMixInfo->Draw("HIST");
      AliMixEventPool2 *evPool = (AliMixEventPool2 *) fMixInfo->GetEventPool("mixEventPool");
      if (evPool) evPool->Print();
   }
}

//_____________________________________________________________________________
void AliAnalysisTaskMixInfo2::SetLogType(AliLog::EType_t type, TString allClasses)
{
   //
...
//_____________________________________________________________________________
void AliAnalysisTaskMixInfo2::SetDebugForAllClasses()
{
   //
...
//_____________________________________________________________________________
void AliAnalysisTaskMixInfo2::InitMixInfo()
{
   //
   // Init mixing info
   //
   if (fInputEHMix) {
      fMixInfo = new AliMixInfo2("mixInfo", "Mix title");
      AliMixEventPool2 *evPool = fInputEHMix->GetEventPool();
      if (!evPool) {
         //             TList *list = new TList;
         if (fMixInfo) fMixInfo->CreateHistogram(AliMixInfo2::kMainEvents, 1, 1, 2);
         if (fMixInfo) fMixInfo->CreateHistogram(AliMixInfo2::kMixedEvents, 1, 1, 2);
      } else {
         if (evPool->NeedInit()) evPool->Init();
         Int_t num = evPool->GetListOfEntryLists()->GetEntriesFast();
         if (fMixInfo) fMixInfo->CreateHistogram(AliMixInfo2::kMainEvents, num, 1, num + 1);
         if (fMixInfo) fMixInfo->CreateHistogram(AliMixInfo2::kMixedEvents, num, 1, num + 1);
      }
   }
}

//_____________________________________________________________________________
void AliAnalysisTaskMixInfo2::PrintEventInfo()
{
   //
...
//_____________________________________________________________________________
void AliAnalysisTaskMixInfo2::InitInputHandlers()
{
   //
   ...
   if (fInputEHMain) {
      fInputEHMix = dynamic_cast<AliMixInputEventHandler2 *>(fInputEHMain->GetFirstMultiInputHandler());
   }
}
