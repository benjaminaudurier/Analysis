...
#ifndef ALIMIXINPUTEVENTHANDLER2_H
#define ALIMIXINPUTEVENTHANDLER2_H
...
class AliMixInputEventHandler2 : public AliMultiInputEventHandler {

public:
   AliMixInputEventHandler2(const Int_t size = 1, const Int_t mixNum = 1);
   virtual ~AliMixInputEventHandler2();
   // From the interface
   ...
   void                    SetEventPool(AliMixEventPool2 *const evPool) { fEventPool = evPool; }

   AliMixEventPool2        *GetEventPool() const { return fEventPool; }
...
protected:
   ...
   AliMixEventPool2        *fEventPool;             // event pool
...
private:
   ...
   AliMixInputEventHandler2(const AliMixInputEventHandler2 &handler);
   AliMixInputEventHandler2 &operator=(const AliMixInputEventHandler2 &handler);

   ClassDef(AliMixInputEventHandler2, 5)
};
