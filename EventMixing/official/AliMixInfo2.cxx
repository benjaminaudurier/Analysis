...
ClassImp(AliMixInfo2)

//_________________________________________________________________________________________________
AliMixInfo2::AliMixInfo2(const char *name, const char *title) :
   TNamed(name, title),
   fHistogramList(0)
{
   //
...
//_________________________________________________________________________________________________
AliMixInfo2::AliMixInfo2(const AliMixInfo2 &obj) :
   TNamed(obj),
...
//_________________________________________________________________________________________________
void AliMixInfo2::CreateHistogram(AliMixInfo2::EInfoHistorgramType type, Int_t nbins, Int_t min, Int_t max)
{
   //
...
//_________________________________________________________________________________________________
void AliMixInfo2::FillHistogram(AliMixInfo2::EInfoHistorgramType type, Int_t value)
{
   //
   // Create mix info histograms
   //
...
//_________________________________________________________________________________________________
const char *AliMixInfo2::GetNameHistogramByType(Int_t index) const
{
   //
   // Retruns name of cut
   //
...
//_________________________________________________________________________________________________
const char *AliMixInfo2::GetTitleHistogramByType(Int_t index) const
{
   //
   // Retruns name of cut
   //
...
//_________________________________________________________________________________________________
void AliMixInfo2::Print(Option_t *option) const
{
   //
   // Print Mix info
   //
...
//_________________________________________________________________________________________________
void AliMixInfo2::Draw(Option_t *option)
{
   //
...
//     upperPad1->cd(1);
   ...
   TH2I *hMixInfo2D = 0;
//     TH1I *hOK=0,*hBad=0;
   AliMixEventPool2 *evPool = (AliMixEventPool2 *) GetEventPool("mixEventPool");
   if (evPool) {
...
//     legend->Draw();
   ...
   cMain->cd(2)->AddExec("dynamic", Form("AliMixInfo2::DynamicExec((AliMixInfo2*)0x%lx)", (ULong_t)this));
}

//_________________________________________________________________________________________________
void AliMixInfo2::DynamicExec(AliMixInfo2 *const mixInfo)
{
   //
...
//    Int_t mixNum = 1;
   if (text) {
      if (mixInfo) {
         AliMixEventPool2 *evPool = (AliMixEventPool2 *) mixInfo->GetEventPool("mixEventPool");
         if (evPool) {
...
//_________________________________________________________________________________________________
Long64_t AliMixInfo2::Merge(TCollection *list)
{
   //
   // Merge function
   //
   ...
   AliMixInfo2 *mi = 0;
   Long64_t counter = 0;
   while ((mi = (AliMixInfo2 *) nxfc())) {
      // Do not merge with ourself
      if (mi == this) continue;
      // Make sure that it is a AliMixInfo2
      if (!mi->InheritsFrom(AliMixInfo2::Class())) {
...
TH1I *AliMixInfo2::GetHistogramByType(Int_t index) const
{
   //
   // GetHistogramByType
   //
...
//_________________________________________________________________________________________________
void AliMixInfo2::Add(AliMixInfo2 *mi)
{
   //
   // adds AliMixInfo2
   //
...
//_________________________________________________________________________________________________
void AliMixInfo2::SetEventPool(AliMixEventPool2 *evPool)
{
   //
   // Sets event pool
   //
...
//_________________________________________________________________________________________________
AliMixEventPool2 *AliMixInfo2::GetEventPool(const char *name)
{
   //
   // Gets event pool
   //
   ...
   return (AliMixEventPool2 *) fHistogramList->FindObject(name);
}
