...
TString remoteDir =Form("/alice/cern.ch/user/b/baudurie/Analysis/LHC15g/TrackingEfficiency/simsinglemuon/pp/%s",Trigger.Data());
...
void runAccEffSubmitter(const char* mode)
{
	...
	a.MakeNofEventsPropToTriggerCount(Trigger.Data(),5);
	a.SetVar("VAR_GENLIB_PARNAME","\"pp 5.03\"");
	a.SetRunList("runlist_15-07-2015-ESD.txt");
	a.SetPackages("VO_ALICE@AliPhysics::vAN-20150722");
	a.SetCompactMode(0);
	a.Print();
	a.Run(mode);
	// a.Submit(kFALE)
