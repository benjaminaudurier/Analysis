...
TString aliphysicsVersion = "vAN-20150708";
...
AliAnalysisTask* runMuMu(const char* dataset="dataset_15-07-2015-AOD.txt",
                         ...
                         TString where="full",const char* alirootMode="")
{
    ...
    else if (mode == kGrid || mode == kTerminate) 
    {
        ...
        alienHandler = static_cast<AliAnalysisGrid*>(CreateAlienHandler(where, alirootVersion, aliphysicsVersion, dataset, dataDir, dataPattern, outDir, extraLibs, extraIncs, extraTasks, analysisMacroName, runFormat, ttl, maxFilesPerJob, maxMergeFiles, maxMergeStages));
  ...
  if (!simulations)
  {
     ...
     triggers->Add(new TObjString("CINT7-B-NOPF-CENTNOTRD || CINT7-B-NOPF-MUON"));// MB 
