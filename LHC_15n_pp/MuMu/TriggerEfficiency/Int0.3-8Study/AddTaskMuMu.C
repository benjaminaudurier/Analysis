                             ...
                             Bool_t simulations)
{
  ...
  AliAnalysisMuMuMinv  * minvAnalysis = new AliAnalysisMuMuMinv(0x0,2);// Analysis dealing with single muon
  ...
  if ( singleAnalysis )
  {
    ...
    cr->AddCutCombination(trackTrue,rabs,eta,pdca);
     ...
     if ( minvAnalysis )
    {
      ...
      // Weight on muon tracks
      minvAnalysis->SetMuonWeight();
      ...
      AliAnalysisMuMuCutElement* ptRange  = cr->AddTrackPairCut(*minvAnalysis,"IsPtInRange","const AliVParticle&,const AliVParticle&,Double_t&,Double_t&","0.3,8.");
