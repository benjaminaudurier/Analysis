...
char                * wfile="AnalysisResults.JPSI.DATAReference.root";
char                * reffile="AnalysisResults.JPSI.MCReference.root";
char                * beamYear="mumu.pp2015.simu.nomatchlow.config";
TString spectraPath    = "/ALL/ANY/PP/pALLPAIRYPAIRPTIN0.3-8.0RABSETAPDCA";
...
void CompareTriggerEfficiency( TString what ="pt", int debug =0  )
{
	...
	if(what.Contains("integrated")) mcspectra  = static_cast<AliAnalysisMuMuSpectra*>(aref->OC()->GetObject(Form("%s/PSI-INTEGRATED",spectraPath.Data())));
    if(!mcspectra){
