...
void runMuMuFacilities(TString smode = "local", TString inputFileName = "AliAOD.Muons.root", Bool_t isMC = kFALSE, Int_t AcceffFileNumber)
{
  ...
  if (mode == kSAF3Connect) {
    ...
    CreateAnalysisTrain(isMC,AcceffFileNumber);
...
//______________________________________________________________________________
void CreateAnalysisTrain(Bool_t isMC,Int_t AcceffFileNumber)
{
  ...
  AliAnalysisTaskMuMu*         TaskMuMu = AddTaskMuMu(output.Data(),"pp2015",isMC,AcceffFileNumber);
