    ...
    while ( ( scent = static_cast<TObjString*>(nextCent()) ) ){
        ...
        if(swhat->String().Contains("PT") || swhat->String().Contains("Y"))
        {
           ...
           // spectra->Plot("NofJPsi","",kTRUE)->DrawCopy("");
            if(swhat->String().Contains("PT")) ana.ComputePPCrossSection(swhat->String().Data(),"PSI","CorrNofJPsi","externFile_PT.txt","externFile_CENT.txt",print);
            else ana.ComputePPCrossSection(swhat->String().Data(),"PSI","CorrNofJPsi","externFile_Y.txt","externFile_CENT.txt",print);
