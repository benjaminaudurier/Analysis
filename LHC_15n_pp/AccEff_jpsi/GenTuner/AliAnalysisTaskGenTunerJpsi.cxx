...
if (nofptbin==0 || nofybin==0 )
{
  ...
  TH1* hPtGen =  (TH1F*)fHptRef->Clone("hPtGen");
  hPtGen->Reset();
  hPtGen->SetTitle("generated p_{T} distribution;p_{T} (GeV/c);dN/dp_{T}");
  ...
  TH1* hPtRec =  (TH1F*)fHptRef->Clone("hPtRec");
  hPtRec->Reset();
  hPtRec->SetTitle("reconstructed p_{T} distribution;p_{T} (GeV/c);dN/dp_{T}");
  ...
  TH1* hYGen =  (TH1F*)fHyRef->Clone("hYGen");
  hYGen->Reset();
  hYGen->SetTitle("generated y distribution;y;dN/dy");
  hYGen->Sumw2();// Create structure to store sum of squares of weights
  fList->AddAtAndExpand(hYGen, kYGen);// Double array size if reach the end
  

  TH1* hYRec =  (TH1F*)fHyRef->Clone("hYRec");
  hYRec->Reset();
  hYRec->SetTitle("reconstructed y distribution;y;dN/dy");
  hYRec->Sumw2();// Create structure to store sum of squares of weights
  fList->AddAtAndExpand(hYRec, kYRec);// Double array size if reach the end


  // TH1* hPtGen = new TH1D("hPtGen","generated p_{T} distribution;p_{T} (GeV/c);dN/dp_{T}",nofptbin,fPtBin[0],fPtBin[1]);
  // hPtGen->Sumw2();// Create structure to store sum of squares of weights
  // fList->AddAtAndExpand(hPtGen, kPtGen);// Double array size if reach the end
  // TH1* hPtRec = new TH1D("hPtRec","reconstructed p_{T} distribution;p_{T} (GeV/c);dN/dp_{T}",nofptbin,fPtBin[0],fPtBin[1]);
  // hPtRec->Sumw2();// Create structure to store sum of squares of weights
  // fList->AddAtAndExpand(hPtRec, kPtRec);// Double array size if reach the end
  
  // TH1* hYGen = new TH1D("hYGen","generated y distribution;y;dN/dy",nofybin,fYBin[0],fYBin[1]);
  // hYGen->Sumw2();
  // fList->AddAtAndExpand(hYGen, kYGen);
  // TH1* hYRec = new TH1D("hYRec","reconstructed y distribution;y;dN/dy",nofybin,fYBin[0],fYBin[1]);
  // hYRec->Sumw2();
...
void AliAnalysisTaskGenTunerJpsi::Terminate(Option_t *)
{
  ...
  fitRangeMC[0][0] = fHptRef->GetXaxis()->GetXmin();
  fitRangeMC[0][1] = fHptRef->GetXaxis()->GetXmax();
  fitRangeMC[1][0] = fHyRef->GetXaxis()->GetXmin() ;
  fitRangeMC[1][1] = fHyRef->GetXaxis()->GetXmax() ;
