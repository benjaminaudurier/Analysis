...
void MuonTrackingEfficiency(TString runList = "runList.txt",
                            ...
                            TString pathForRunResults = "alice/cern.ch/user/b/baudurie/Analysis/LHC15n/TrackingEfficiency/MonteCarlo/singleMuon/tuned/WiththnSparse/results")
{
                                     ...
                                     TString fileNameData, TString fileNameSave, Bool_t print, Bool_t draw, const char *spath)
{
  ...
  while (!inFile.eof()) {
    ...
    TString dataFile = Form("%s/%06d/%s",spath, run, fileNameData.Data());
    TString outFile = Form("%s/%06d/%s",spath, run, fileNameSave.Data());
