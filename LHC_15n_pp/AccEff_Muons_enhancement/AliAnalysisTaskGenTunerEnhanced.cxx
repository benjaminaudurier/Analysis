...
#include "AliAnalysisTaskGenTunerEnhanced.h"


ClassImp(AliAnalysisTaskGenTunerEnhanced)

//________________________________________________________________________
AliAnalysisTaskGenTunerEnhanced::AliAnalysisTaskGenTunerEnhanced() :
AliAnalysisTaskSE(),
fList(0x0),
fCentMin(-FLT_MAX),
fCentMax(FLT_MAX),
...
//________________________________________________________________________
AliAnalysisTaskGenTunerEnhanced::AliAnalysisTaskGenTunerEnhanced(const char *name) :
...
void AliAnalysisTaskGenTunerEnhanced::UserCreateOutputObjects()
{
  ...
  // Histo to store number of event only on data
  if ( fDataFile && !fhEvtVsRun ){ // Meaning "not running on MC or not supporsed to"
    TH1* fhEvtVsRun = new TH1D();
    fhEvtVsRun->SetTitle("Number of Event per run in data file");
    fList->AddAtAndExpand(fhEvtVsRun,kEvtVsRun);
  }
...
//________________________________________________________________________
void AliAnalysisTaskGenTunerEnhanced::NotifyRun()
{
...
//________________________________________________________________________
void AliAnalysisTaskGenTunerEnhanced::UserExec(Option_t *)
{
  /// process event
  ...
  if(fDataFile) MakeRunWeight(fDataFile,frunWeights);
  ...
  if (MCEvent()) {
    ...
    for (Int_t i = 0; i < MCEvent()->GetNumberOfTracks(); i++) {
      ...
      //_____compute the weights for all primary particles (other weights are 0)_____
      ...
      if (fGenPtCut > 0. && pT < fGenPtCut) weight[i] = 0.;
        ...
        TString currRun =runNumber;     
        
        // get run trigger scale
        Double_t triggsc;
        
        if (frunWeights){
          TParameter<Double_t> *scale;
          scale   = static_cast<TParameter<Double_t>*>(frunWeights->FindObject(currRun.Data()));
          triggsc = scale->GetVal();
        } else {
          triggsc= 1.;
        }
        
        // Get the total number of tracks in data from ref trigger
        Double_t nTracksBin;
        Double_t nTracks; 
        if(fhEvtVsRun) nTracksBin = fhEvtVsRun->FindBin(runNumber);
        if(nTracksBin != 0.)nTracks = fhEvtVsRun->GetBinContent(runNumber);

        //correct weight
        if(nTracks!=0.) weight[i] = weight[i] / ( nTracks * (1./triggsc) *100.  );
  ...
  for (Int_t i = 0; i < aod->GetNumberOfTracks(); i++){
    ...
    if (!MCEvent())((TH1*)fList->UncheckedAt(kEvtVsRun))->Fill(runNumber);//fill only data
...
//________________________________________________________________________
void AliAnalysisTaskGenTunerEnhanced::Terminate(Option_t *)
{
  /// post-processing
  ...
  TF1 *ptRat = (hRat[0] && fPtFunc && fPtFuncNew) ? new TF1("ptRat", this, &AliAnalysisTaskGenTunerEnhanced::PtRat, fitRangeMC[0][0], hRat[0]->GetXaxis()->GetXmax(), 0, "AliAnalysisTaskGenTunerEnhanced", "PtRat") : 0x0;
  TF1 *yRat = (hRat[1] && fYFunc && fYFuncNew) ? new TF1("yRat", this, &AliAnalysisTaskGenTunerEnhanced::YRat, fitRangeMC[1][0], fitRangeMC[1][1], 0, "AliAnalysisTaskGenTunerEnhanced", "YRat") : 0x0;
...
//________________________________________________________________________
void AliAnalysisTaskGenTunerEnhanced::SetOriginPtFunc(TString formula, const Double_t *param, const Bool_t *fixParam,
...
//________________________________________________________________________
void AliAnalysisTaskGenTunerEnhanced::SetNewPtFunc(TString formula, const Double_t *param, const Bool_t *fixParam,
                                           Double_t xMin, Double_t xMax)
{
...
//________________________________________________________________________
void AliAnalysisTaskGenTunerEnhanced::SetOriginYFunc(TString formula, const Double_t *param, const Bool_t *fixParam,
                                             Double_t xMin, Double_t xMax)
{
...
//________________________________________________________________________
void AliAnalysisTaskGenTunerEnhanced::SetNewYFunc(TString formula, const Double_t *param, const Bool_t *fixParam,
                                          Double_t xMin, Double_t xMax)
{
...
//________________________________________________________________________
TH1* AliAnalysisTaskGenTunerEnhanced::ComputeAccEff(TH1 &hGen, TH1 &hRec, const Char_t *name, const Char_t *title)
{
...
//________________________________________________________________________
Double_t AliAnalysisTaskGenTunerEnhanced::GetFitLowEdge(TH1 &h)
{
...
//________________________________________________________________________
Double_t AliAnalysisTaskGenTunerEnhanced::GetFitUpEdge(TH1 &h)
{
...
//________________________________________________________________________
void AliAnalysisTaskGenTunerEnhanced::NormFunc(TF1 *f, Double_t min, Double_t max)
{
...
//________________________________________________________________________
Double_t AliAnalysisTaskGenTunerEnhanced::PtRat(const Double_t *x, const Double_t */*p*/)
{
...
//________________________________________________________________________
Double_t AliAnalysisTaskGenTunerEnhanced::YRat(const Double_t *x, const Double_t */*p*/)
{
...
//---------------------------------------------------------------------------
void AliAnalysisTaskGenTunerEnhanced::MakeRunWeight(TString name, THashList*runWeights )
{
  /// Set the trigger scale events per run
  if (runWeights) return;
  
  ifstream inFile(name.Data());
  if (!inFile.is_open()) {
    printf("cannot open file %s", name.Data());
    return;
  }
