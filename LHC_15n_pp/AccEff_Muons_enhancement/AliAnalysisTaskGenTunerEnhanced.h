...
/// \ingroup muondep
/// \class AliAnalysisTaskGenTunerEnhanced
...
class AliAnalysisTaskGenTunerEnhanced : public AliAnalysisTaskSE {
public:
  
  AliAnalysisTaskGenTunerEnhanced();
  AliAnalysisTaskGenTunerEnhanced(const char *name);
  virtual ~AliAnalysisTaskGenTunerEnhanced();
  ...
  /// Select Event vs Run Histo
  void SetEvtVsRunHisto(TH1* hEvtVsRun) {if(hEvtVsRun) fhEvtVsRun = static_cast<TH1*>(hEvtVsRun->Clone());}
  ...
  // set the name of the data file used in userexec to tune the generated distributions
  void SetScaleFile(const Char_t* name) {fScaleFile = name;}
  ...
  // Make frunWeights from fScaleFile
  void MakeRunWeight(TString name, THashList*runWeights );
  
private:
  
  /// Not implemented
  AliAnalysisTaskGenTunerEnhanced(const AliAnalysisTaskGenTunerEnhanced& rhs);
  /// Not implemented
  AliAnalysisTaskGenTunerEnhanced& operator = (const AliAnalysisTaskGenTunerEnhanced& rhs);
...
private:
  ...
  enum eList {
    ...
    kEvtVsRun = 6  ///< Event versus run distribution of reconstructed particle
  };
  ...
  // _____ Data Member specific to AliAnalysisTaskGenTunerEnhanced -----
  TString   fScaleFile;              ///< File with the scale between two trigger. line must be : <runnumber> <scale in %>
  TH1       *fhEvtVsRun;             ///< Counter of the number of tracks per run
  THashList *frunWeights;             ///< Here we store the relative scale between triggers

  ClassDef(AliAnalysisTaskGenTunerEnhanced, 2);
};

//________________________________________________________________________
inline void AliAnalysisTaskGenTunerEnhanced::SetMuonTrackCuts(AliMuonTrackCuts &trackCuts)
{
  /// set standard cuts to select tracks to be considered
