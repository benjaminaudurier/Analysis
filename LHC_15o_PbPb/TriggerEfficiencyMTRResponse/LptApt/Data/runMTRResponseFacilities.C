...
///
  ...
  TString aliphysicsVersion = "vAN-20170106-1";
  TString extraLibs         ="";
  ...
  TString analysisMacroName = "MTRResp";
  Int_t ttl                 = 30000;
...
void runMTRResponseFacilities(TString smode = "local", TString inputFileName = "AliAOD.Muons.root", Bool_t isMC = kFALSE)
{
  /// Fill counters with triggered events

  TList pathList; pathList.SetOwner();
  TList fileList; fileList.SetOwner();
  fileList.Add(new TObjString( "runMTRResponseFacilities.C" ));
  fileList.Add(new TObjString( "AddTaskMTRResponseAnalysis.C" ));
...
void CreateAnalysisTrain(Bool_t isMC)
{
  ...
  gROOT->LoadMacro("AddTaskMTRResponseAnalysis.C");
  AliAnalysisTaskMTRResponse* MTRTask = AddTaskMTRResponseAnalysis(isMC);
