...
TGraphAsymmErrors* CreateRatioGraph(const char* name, const char* title, TGraphAsymmErrors& Graph1, TGraphAsymmErrors& Graph2) // Change this function as in RealEfficiency.C (value)
{
  ...
  ratio->GetYaxis()->SetTitle("Ratioception");
...
{ 
  ...
  TCanvas *c = new TCanvas(name.Data(), "ratioception",1200,800);
...
//-------------------------------------------------------------------------------------------------------------------------------------------
void ComparisonceptionEfficiency(TString f1 = "", TString f2 ="")
{
  // Open input file1
  ...
  TObjArray *listGlobalEff1 = static_cast<TObjArray*>(file1->FindObjectAny(Form("%sEffComp",hname.Data())));
  if (!listGlobalEff1) {
    printf("list of Global efficiencies from 1 not found\n");
    return;
  }
  
  TObjArray *listGlobalEff2 = static_cast<TObjArray*>(file2->FindObjectAny(Form("%sEffComp",hname.Data())));
  if (!listGlobalEff2) {
...
//  globalRatios.Add(CreateRatioGraph("RatioEffVsCent","file1/file2 tracking efficiency versus centrality",effVScent1,effVScent2));
  ...
  TGraphAsymmErrors *ratioRun = CreateRatioGraph("RatioceptionEffVsRun","file1/file2 tracking efficiency versus run",*effVSrun1,*effVSrun2);
  globalRatios.Add(ratioRun);
  
  TGraphAsymmErrors* ratioRunCopy = static_cast<TGraphAsymmErrors*>(ratioRun->Clone());
  
  globalRatiosAndEff.Add(DrawRatio("RatioceptionEffVSRunAndEff","Comparison file1&file2 tracking efficiencyception versus run", effVSrun1Copy,effVSrun2Copy,ratioRunCopy));
  //-----
  ...
  TGraphAsymmErrors *ratioY = CreateRatioGraph("RatioceptionEffVsY","file1/file2 tracking efficiencyception versus rapidity",*effVSy1,*effVSy2);
  globalRatios.Add(ratioY);
  
  TGraphAsymmErrors* ratioYCopy = static_cast<TGraphAsymmErrors*>(ratioY->Clone());
 
  globalRatiosAndEff.Add(DrawRatio("RatioceptionEffVSyAndEff","Comparison file1&file2 tracking efficiencyception versus rapidity", effVSy1Copy,effVSy2Copy,ratioYCopy));
  //-----
  ...
  TGraphAsymmErrors *ratioPt = CreateRatioGraph("RatioceptionEffVsPt","File1/File2 tracking efficiencyception versus Pt",*effVSpt1,*effVSpt2);
  globalRatios.Add(ratioPt);
  
  TGraphAsymmErrors* ratioPtCopy = static_cast<TGraphAsymmErrors*>(ratioPt->Clone());
  
   globalRatiosAndEff.Add(DrawRatio("RatioceptionEffVSptAndEff","Comparison file1&file2 tracking efficiencyception versus Pt",effVSpt1Copy,effVSpt2Copy,ratioPtCopy));
  //-----
  ...
  TGraphAsymmErrors *ratioPhi = CreateRatioGraph("RatioceptionEffVsPhi","file1/file2 tracking efficiencyception versus phi",*effVSphi1,*effVSphi2);
  globalRatios.Add(ratioPhi);
  
  TGraphAsymmErrors* ratioPhiCopy = static_cast<TGraphAsymmErrors*>(ratioPhi->Clone());
  
  globalRatiosAndEff.Add(DrawRatio("RatioceptionEffVSphiAndEff","Comparison file1&file2 tracking efficiencyception versus phi",effVSphi1Copy,effVSphi2Copy,ratioPhiCopy));
  //-------
...
//  }
  ...
  TObjArray *listChEffVSDE1 = static_cast<TObjArray*>(file1->FindObjectAny(Form("%sEffperDEComp",hname2.Data())));
  if (!listChEffVSDE1) {
    printf("list of Chamber efficiencies per DE from 1 not found\n");
    return;
  }
  TObjArray *listChEffVSDE2 = static_cast<TObjArray*>(file2->FindObjectAny(Form("%sEffperDEComp",hname2.Data())));
  if (!listChEffVSDE2) {
...
//    }
    ...
    TString name =  Form("DiffRatioceptionEffCh%dVsDE",ich+1); TString title = Form("Chamber %d file1/file2 tracking efficiencyception versus DE",ich+1);
    //----Eff of CHs vs DE
    ...
    TString nameRatio =  Form("RatioceptionEffCh%dVsDEAndEff",ich+1); TString titleRatio = Form("Comparison file1&file2 Ch%d tracking efficiencyception versus DE",ich+1);
...
//    chamberVSdeRatios.Add(CreateRatioGraph(name.Data(),title.Data(),*g1,*g2));
  ...
  //Beautify graphs
  BeautifyGraphs(globalRatios,"Effception1/Eff2ception");
//  BeautifyGraphs(deVSrunRatios,"Eff1/Eff2");
//  BeautifyGraphs(chamberVSrunRatios,"Eff1/Eff2");
  BeautifyGraphs(chamberVSdeRatios,"Eff1ception/Eff2ception");
...
//  
  // save output
  TFile* file = new TFile("ComparisonceptionEfficiency.root","update");
  
  globalRatios.Write("GlobalEffCompception", TObject::kOverwrite | TObject::kSingleKey);
...
//  deVSrunRatios.Write("DEEffVSrunRatios", TObject::kOverwrite | TObject::kSingleKey);
  chamberVSdeRatios.Write("ChamberEffperDECompception", TObject::kOverwrite | TObject::kSingleKey);
  
  globalRatiosAndEff.Write("GlobalEffCompceptionAndEffs", TObject::kOverwrite | TObject::kSingleKey);
...
//  deVSrunRatiosAndEff.Write("DEEffVSrunRatiosAndEff", TObject::kOverwrite | TObject::kSingleKey);
  chamberVSdeRatiosAndEff.Write("ChamberEffperDECompceptionAndEff", TObject::kOverwrite | TObject::kSingleKey);
