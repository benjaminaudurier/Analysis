                             ...
                             Bool_t simulations)
{
  ...
  task->SelectCollisionCandidates(AliVEvent::kMUSPB);
  ...
  if ( singleAnalysis ){
    ...
    if ( triggResponse )
    {
      char* LptAptCutName = "sALLRABSMATCHLOWETAPDCA";
      triggResponse->SetPathLptAptHisto(LptAptCutName); // name of the lpt cut
      triggResponse->SetProjectionAxis(1);           // projection on the pt axis
