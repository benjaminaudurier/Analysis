                             ...
                             Bool_t simulations)
{
  ...
  TFile *f = TFile::Open("ReferenceResults_Data.root");
  f->GetObject("OC",coll);
  if(!coll)
  {
  ...
  ps  = cr->AddEventCut(*eventCutter,"IsPhysicsSelectedINT7inMUON","const AliInputEventHandler&","");
  ...
  if ( singleAnalysis ){
    ...
    cr->AddCutCombination(trackTrue,rabs,eta,pdca,matchany);
    ...
    if ( triggResponse )
    {
      Double_t etarange[11] = {-4.,-3.85,-3.70,-3.55,-3.40,-3.25,-3.10,-2.95,-2.80,-2.65,-2.5};
      triggResponse->SetLptAptEtaRange(etarange,11);
