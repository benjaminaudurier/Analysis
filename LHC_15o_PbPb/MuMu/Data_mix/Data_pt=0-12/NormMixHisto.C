...
TString dir       = "/Users/audurier/Documents/Analysis/LHC_15o_PbPb/MuMu/Data_mix/Data_pt=0-12/results";
    ...
    while ( ( sdirs = static_cast<TObjString*>(nextdirs()) ) ) {
        ...
        while ( ( sbintype = static_cast<TObjString*>(nextbintype()) ) ){
            if(sbintype->String().Contains("integrated")){
                analysis->NormarliseMixedMinv(sbintype->String().Data(),"psi","");
            } else {
                analysis->NormarliseMixedMinv(sbintype->String().Data(),"psi","BENJ");
            }
        }
    }
