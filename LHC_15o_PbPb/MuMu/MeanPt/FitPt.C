...
//------------------------------------------------------------------
void FitPt(const char* input="dsigmadydpt_pp.txt" , Bool_t systptcut = kFALSE, Int_t errorType = 2, Double_t delta = 0.1)
{
  // norm = kTRUE if yields have been divided by pT
  // errorType: stat = 0, sys = 1, all = other int
  // delta: range for <pT> scan to <pT> ± delta*<pT>
  ...
  while (in.getline(line,1023,'\n'))
  {
    ...
    if (systptcut) {
      // apply weight to the first bin to take into account the photo-production according to centrality



      TString sinput(input);
      Double_t s = sinput.Contains("50-60") ? 0.97 : 1.;
      if(sinput.Contains("60-90")) s = 0.91;


      if(a == 0.5 && b == 1.0)
      {
        printf("Applying pt cut correction s = %.2f\n",s);
  ...
  TString strinp("input");
  if (strinp.Contains("AuAu200")){
    f->SetParameter(0,0001);
  ...
  TFitResultPtr r = h->Fit(f,"0QIRS","",xminfit,xmaxfit);
  f->Draw("same");
